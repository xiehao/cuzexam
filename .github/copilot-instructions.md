...
- `sheetcontents` is not auto-generated from the problems. For answer sheets, authors manually build the mirrored tables or lists, often with `\repeatcell` and `\lastvalue` to continue numbering across rows.
- For centralized answer areas, define reusable answer keys with `\cuzanswerdef{key}{value}` outside `problem`, then reference them with helpers like `\pickoutref`, `\pickoutxref`, `\fillindexref`, and `\fillinxref`. This placement matters in `answer,sheet` mode because `problem` environments are excluded entirely.
